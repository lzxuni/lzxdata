...
(function () {
    ...
    if (l.$wdate) {
        ...
        m(urls+"/Content/datepicker/skin/WdatePicker.css")
    }
    E = n;
    if (l.$crossFrame) {
        try {
    ...
    function a(e, d) {
        ...
        function w(V, P) {
            ...
            function R() {
                ...
                V.lang = Y.name;
                V.skin = P.skin;
                var X = ["<head><script>", "", "var doc=document, $d, $dp, $cfg=doc.cfg, $pdp = parent.$dp, $dt, $tdt, $sdt, $lastInput, $IE=$pdp.ie, $FF = $pdp.ff,$OPERA=$pdp.opera, $ny, $cMark = false;", "if($cfg.eCont){$dp = {};for(var p in $pdp)$dp[p]=$pdp[p];}else{$dp=$pdp;};for(var p in $cfg){$dp[p]=$cfg[p];}", "doc.oncontextmenu=function(){try{$c._fillQS(!$dp.has.d,1);showB($d.qsDivSel);}catch(e){};return false;};", "<\/script><script src=", urls,"/Content/datepicker/lang/", Y.name, ".js charset=", Y.charset, "><\/script>"];
                if (S) {
                    X[1] = 'document.domain="' + O + '";'
                }
                for (var W = 0; W < U.length; W++) {
                    if (U[W].name == P.skin) {
                        X.push('<link rel="stylesheet" type="text/css" href="'+urls+'/Content/datepicker/skin/default/datepicker.css" charset="' + U[W].charset + '"/>')
                    }
                }
                //X.push('<script src="' + u + 'calendar.js"><\/script>');
                X.push('<script src="'+urls+'/Content/datepicker/calendar.js"><\/script>');
